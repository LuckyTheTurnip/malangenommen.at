document.addEventListener('DOMContentLoaded', () => {
	...
	const MAX_TILT_X = 11
	const MAX_TILT_Y = 13
	const MAX_MOTION_X = 16
	const MAX_MOTION_Y = 18
	const TILT_DAMPING = 0.12
	...
	const applyMotionTransform = () => {
		const target = tiltEl || activeCard
	...
	const updateTiltFromOrientation = (event) => {
		...
		const targetMotionX = Math.max(-MAX_MOTION_X, Math.min(MAX_MOTION_X, beta * 0.18))
		const targetMotionY = Math.max(-MAX_MOTION_Y, Math.min(MAX_MOTION_Y, -gamma * 0.2))
